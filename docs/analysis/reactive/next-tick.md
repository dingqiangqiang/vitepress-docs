...
相关链接: [这一次，彻底弄懂 JavaScript 执行机制](https://juejin.cn/post/6844903512845860872)
...
## 扩展
一道经典面试题来测测你是否已经掌握了 `Javascript` 的事件循环机制
...
console.log(5)
// 执行顺序为: 1 -> 2 -> 3 -> 4 -> 5 -> 7 -> 6 -> 8
```
:::warning 提示
`await async2(); console.log(7)` 会被编译成如下代码:
```js
// console.log(7) 会被放在 await 语法糖生成的 promise.then 里
Promise.resolve(console.log(3)).then(() => { console.log(7) })
```
:::
